...
- **小红书详情页抓取较慢**：每条笔记需单独请求详情页获取正文，含间隔约5秒/条，15条约需75秒。多款竞品时整体耗时较长。
- **小红书正文偶有缺失**：详情页 API 偶尔返回空数据，该条正文为空，不影响其他字段但词云数据可能不完整。
- **小红书 Cookie 有效期短**：约1-2周过期，过期后需手动重新获取并更新 `.env`。
- **无重试机制**：网络波动时请求失败直接跳过，不自动重试，可能丢数据。

### 文案生成

- **依赖 agent 模型能力**：文案质量取决于 agent 对竞品原文的研读和模仿能力，不同模型表现差异较大。
- **文案需人工审核**：生成的文案为初稿，发布前需人工校对内容准确性和品牌调性。
